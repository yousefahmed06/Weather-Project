...
def load_existing_keys(path: str) -> set[tuple[str, str]]:
    """Read (city, forecast_datetime) pairs already present in the CSV,
    ...
    try:
        with open(path, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                existing_keys.add((row["city"], row["forecast_datetime"]))
        print(f"Loaded {len(existing_keys)} existing records.")
    except Exception as e:
        print(f"Could not read existing CSV for dedup, starting fresh: {e}")
    ...
    with open(CSV_FILE, "a", newline="", encoding="utf-8") as f:
        ...
        for i, city in enumerate(CITIES):
            try:
                ...
                skipped = 0
                for row in rows:
                    key = (row["city"], row["forecast_datetime"])
                    if key not in existing_keys:
                        ...
                        skipped += 1
                print(f"[{i + 1}/{len(CITIES)}] {city} \u2713 {added} added, {skipped} duplicate(s) skipped")
            except Exception as e:
